...
<body>
	<main>
		<section class="container wrapper">
			...
			<?php
				if ($_SERVER['REQUEST_METHOD'] === 'POST') {
					...
					if ($_SESSION["value"] === 'Denunciar') {
						$_SESSION["denounces_from_where"] = 5;
			...
			<br/><h3 style="text-align: center;" class="display-5"><strong> <?php echo $_SESSION["quest_5"][1];?> </strong></h3>
			...
			<form method="post">
				...
				<?php
					...
					if (array_key_exists('elimina', $_POST)) {
						...
						class="btn btn-block btn btn-outline-primary" value= "'.$_SESSION["quest_5"][2].'" /> <br/></div>',
						'<div id="divFirst"><input type="submit" name="buttom" 
						class="btn btn-block btn btn-outline-primary" value= "'.$_SESSION["quest_5"][3].'" /> <br/></div>');
						...
						class="btn btn-block btn btn-outline-primary" value= "'.$_SESSION["quest_5"][3].'" /> <br/></div>',
						'<div id="divFirst"><input type="submit" name="buttom" 
						class="btn btn-block btn btn-outline-primary" value= "'.$_SESSION["quest_5"][4].'" /> <br/></div>',
						'<div id="divFirst"><input type="submit" name="buttom" 
						class="btn btn-block btn btn-outline-primary" value= "'.$_SESSION["quest_5"][5].'" /> <br/></div>');
					...
					echo "<br/><br/>";
					echo '<input type="submit" name='.$_SESSION["quest_5"][0].' class="btn btn-block btn btn-outline-danger" value="Denunciar">';
